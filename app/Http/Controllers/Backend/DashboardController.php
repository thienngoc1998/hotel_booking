...
//    }
    ...
    public function confirmBookingRoom(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $availability) {
            ...
            $this->updateRoom($availability['id_room']);
            $totalService = $this->bookingService($request->service, $book->id);
            $book->update([
                'total' => $book->total + $totalService
            ]);
    ...
    private function bookingService($array, $idBook)
    {
        $sum = 0;
        for ($i = 0; $i < count($array); $i++) {
            ...
            $sum += Service::find($array[$i])->price;
        }
        return $sum;
    }
